...
impl BlackAdamW {
    ...
    fn black_init_state(&mut self, black_params: &[Arc<RwLock<BlackVar>>]) -> BlackResult<()> {
        self.black_m.clear();
        self.black_v.clear();
        self.black_v_max.clear();

        for black_p in black_params {
            let black_p_r = black_p.read();
            let black_shape = black_p_r.black_data.black_shape.black_dims();
            let black_m = BlackTensor::black_zeros(
                black_shape,
                black_core::black_dtype::BlackDType::BlackF32,
            )?;
            let black_v = BlackTensor::black_zeros(
                black_shape,
                black_core::black_dtype::BlackDType::BlackF32,
            )?;
            self.black_m.push(black_m.clone());
            self.black_v.push(black_v.clone());
            if self.black_amsgrad {
                self.black_v_max.push(black_v);
            }
        }
...
impl BlackOptimizer for BlackAdamW {
    fn black_step(&mut self, black_params: &mut [Arc<RwLock<BlackVar>>]) -> BlackResult<()> {
        if !self.black_initialized {
            self.black_init_state(black_params)?;
        }

        self.black_step_count += 1;
        let black_t = self.black_step_count as f64;
        let black_bias_correction1 = 1.0 - self.black_beta1.powf(black_t);
        let black_bias_correction2 = 1.0 - self.black_beta2.powf(black_t);
        let black_lr_t =
            self.black_lr * black_bias_correction2.sqrt() / black_bias_correction1;
        ...
        for (black_idx, black_param) in black_params.iter().enumerate() {
            ...
            for black_i in 0..black_param_data.len() {
                ...
                black_v_data[black_i] =
                    black_b2 * black_v_data[black_i] + (1.0 - black_b2) * black_g * black_g;

                let black_denom = if self.black_amsgrad {
                    let black_vm_buf =
                        std::sync::Arc::make_mut(&mut self.black_v_max[black_idx].black_buffer);
                    let black_vm_data = black_vm_buf.black_as_f32_mut_slice();
                    black_vm_data[black_i] = black_vm_data[black_i].max(black_v_data[black_i]);
                    black_vm_data[black_i].sqrt() + black_e
                } else {
                    black_v_data[black_i].sqrt() + black_e
                };

                black_param_data[black_i] -= black_lr_f * black_m_data[black_i] / black_denom;
            }
        }
